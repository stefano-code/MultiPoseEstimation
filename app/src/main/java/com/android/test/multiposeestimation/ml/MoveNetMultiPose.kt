...
import com.android.test.multiposeestimation.data.BodyPart
import com.android.test.multiposeestimation.data.KeyPoint
import com.android.test.multiposeestimation.data.Person
import com.android.test.multiposeestimation.tracker.*
...
class MoveNetMultiPose(
    ...
    companion object {
        ...
        fun create(
            context: Context ): MoveNetMultiPose {
...
//                        options.addDelegate(gpuDelegate)
            return MoveNetMultiPose(
                Interpreter(
                    FileUtil.loadMappedFile(
                        context, "movenet_multipose_fp16.tflite"), options
                ),
            )
        }
    }
    ...
    private fun processInputTensor(bitmap: Bitmap): TensorImage {
        ...
        val inputSizeHeight =
            /*if (type == Type.Dynamic)*/ DYNAMIC_MODEL_TARGET_INPUT_SIZE //SS else inputShape[1]
        val inputSizeWidth =
            /*if (type == Type.Dynamic)*/ DYNAMIC_MODEL_TARGET_INPUT_SIZE //SS else inputShape[2]
